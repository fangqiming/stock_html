          ...
          <div class="currentHoldingTable">
              ...
              <tr v-for="(item,index) in holding">
                <td>
                  <a :href="item.url" class="strongText blockA" target="_blank" rel="noopener noreferrer"
                     style="display: block!important;text-align: center;color: rgb(2, 117, 216)!important;cursor: pointer;font-weight: 400"><u>{{item.name}}</u></a>
                  <span :class="{Red:item.eps !== null}">{{item.stockName}}</span><br>
                  <span :class="{Red:item.eps !== null}">{{item.eps}}</span>
                </td>
                <td>{{item.oldDate}}<br>{{item.amount | setNum}}<br><span
                  :class="{Green:item.type == '做多',SomeRed:item.type == '做空'}">{{item.type}}</span></td>
                <td>
                  <div class="data_box2">{{item.oldPrice | setNum}}<br>{{item.newPrice | setNum}}</div>
                </td>
                <td>
                  <div class="data_box3">{{item.cost | toInt}}<br>{{item.value | toInt}}</div>
                </td>
                <td>
                  <div class="data_box3">
                    <span v-if="item.gain>0"
                          :class="{Red:item.gain<0,Green:item.gain>=0}">+{{item.gain  | toFixed2 }}%<br>+{{item.earning  | toInt}}</span>
                    <span v-else="item.gain<=0" :class="{Red:item.gain<0,Green:item.gain>=0}">{{item.gain  | toFixed2 }}%<br>{{item.earning  | toInt}}</span>
                  </div>
                </td>
              </tr>
              </tbody>
            ...
            <table class="table table-striped table-bordered table-advance curHoldingTable app_table"
              ...
              <tr v-for="(item,index) in holding" :class="item.gain>=0 ? 'success':''">
                ...
                <td>
                  <div class="data_box3">
                    <span v-if="item.gain>0" :class="{Red:item.earning<0,Green:item.earning>=0}">+{{item.gain | toFixed2 }}%<br>{{item.earning | toInt}}</span>
                    <span v-else="item.gain<=0" :class="{Red:item.earning<0,Green:item.earning>=0}">{{item.gain | toFixed2 }}%<br>{{item.earning | toInt}}</span>
                  </div>
                </td>
              </tr>
              </tbody>
