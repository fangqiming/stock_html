        ...
        <div class="row-fluid">
          <div class="span12">
            <div class="list">
              <template>
                <div>
                  <table id="selfStock" class="table table-striped table-bordered table-advance diagnosticStockInfoTb3"
      ...
      <div class="diagnosticStockInfo1">
        ...
        <div class="diagnosticStockBar">
          <form class="form-search">
            <input type="text" class="form-control" v-on:keyup.enter="search" placeholder="输入股票代码或公司名称"
                   v-model="searchCode"/>
            ...
            <div class="list">
              <template>
                <div class="left_content_operate diagnosticResult">
                  <table class="table table-striped  diagnosticStockInfoTb1" contenteditable="false">
                    ...
                    <template v-if="aiIndexBo.score!=null">
                      ...
                      <tr class="">
                        <td class="right_col">AI得分：<span class="streatText3"
                                                         :class="{Green:resultScore<70,Red:resultScore>=70}">{{aiIndexBo.score | setNum}}分</span>
                        </td>
                      </tr>
                      <tr class="">
                      ...
                      <tr class="">
                        <td class="right_col">预期胜率：<span :class="{Green:resultScore<0,Red:resultScore>=0}">{{aiIndexBo.winRate | setNum}}%</span>
                        </td>
                      </tr>
                      <tr class="">
                        <td class="right_col">综合诊断：该股票预期打败 <span :class="{Green:resultScore<0,Red:resultScore>=0}">{{aiIndexBo.beat | setNum}}%</span>的股票
                        </td>
                      </tr>
                      <tr class="">
                        <td class="right_col">该股票持有15天预期收益率范围为：<span
                          :class="{Green:aiIndexBo.minProfitRate<0,Red:aiIndexBo.minProfitRate>=0}">{{aiIndexBo.minProfitRate}}%</span>~
                          <span :class="{Green:aiIndexBo.maxProfitRate<0,Red:aiIndexBo.maxProfitRate>=0}">+{{aiIndexBo.maxProfitRate | setNum}}%</span>
                        </td>
                      </tr>
                    </template>
                ...
                <div class="right_content_finance">
                  <table class="table table-striped table-bordered table-advance diagnosticStockInfoTb1"
                         contenteditable="false">
                    <tbody>
                    <tr class="">
                      <th><h4>财务风险</h4></th>
                    ...
                    <tr class="">
                      <td class="right_col"><span>该股票质押率为：</span><span
                        :class="{Green:financialRiskBo.pledgeRate<0,Red:financialRiskBo.pledgeRate>=0}">{{financialRiskBo.pledgeRate}}%</span>
                      </td>
                    </tr>
                    <tr class="">
                      <td class="right_col"><span>商誉占市值比：</span><span
                        :class="{Green:financialRiskBo.goodwillRate<0,Red:financialRiskBo.goodwillRate>=0}">{{financialRiskBo.goodwillRate}}%</span>
                      </td>
                    </tr>
                    <tr class="">
                      <td class="right_col"><span>公司负债率为：</span><span
                        :class="{Green:financialRiskBo.debtRate<0,Red:financialRiskBo.debtRate>=0}">{{financialRiskBo.debtRate}}%</span>
                      </td>
                    </tr>
                    <tr class="">
                      <td class="right_col"><span>经营现金流占总资产：</span><span
                        :class="{Green:financialRiskBo.netOperateCashFlowRate<0,Red:financialRiskBo.netOperateCashFlowRate>=0}">{{financialRiskBo.netOperateCashFlowRate}}%</span>
                      </td>
                    </tr>
                    </tbody>
                  </table>
                ...
                <div class="operate_finance">
                  <div class="left_content_operate">
                    <table class="table table-striped table-bordered table-advance diagnosticStockInfoTb1"
                      ...
                      <tr class="">
                        <td class="left_col"><span>近5年来平均毛利率：</span><span
                          :class="{Green:operateBo.grossProfitMarginAvg<0,Red:operateBo.grossProfitMarginAvg>=0}"> {{operateBo.grossProfitMarginAvg | setNum}}%</span>，<span>增长速度：</span><span
                          :class="{Green:operateBo.grossProfitMarginGrowthRate<0,Red:operateBo.grossProfitMarginGrowthRate>=0}">{{operateBo.grossProfitMarginGrowthRate | setNum}}</span>
                        </td>
                      </tr>
                      <tr class="">
                        <td class="left_col"><span>近5年来平均ROE：</span><span
                          :class="{Green:operateBo.roeAvg<0,Red:operateBo.roeAvg>=0}"> {{operateBo.roeAvg | setNum}}%</span>，<span>增长速度：</span><span
                          :class="{Green:operateBo.roeGrowthRate<0,Red:operateBo.roeGrowthRate>=0}">{{operateBo.roeGrowthRate | setNum}}</span>
                        </td>
                      </tr>
                      <tr class="">
                        <td class="left_col"><span>近5年来平均营收增长：</span><span
                          :class="{Green:operateBo.operatingRevenueAvg<0,Red:operateBo.operatingRevenueAvg>=0}"> {{operateBo.operatingRevenueAvg | setNum}}%</span>，<span>增长速度：</span><span
                          :class="{Green:operateBo.operatingRevenueGrowthRate<0,Red:operateBo.operatingRevenueGrowthRate>=0}">{{operateBo.operatingRevenueGrowthRate | setNum}}</span>
                        </td>
                      </tr>
                      </tbody>
                    </table>
                  ...
                  <div class="right_content_finance">
                    <table class="table table-striped table-bordered table-advance diagnosticStockInfoTb1"
                      ...
                      <tr class="" v-for="item2 of chooseStock">
                        <td class="right_col"><span>{{item2.title}}</span><span v-for="item3 of item2.value">{{item3}}&nbsp;&nbsp;&nbsp;&nbsp;</span>
                        </td>
                      </tr>
                      </tbody>
                    </table>
              ...
              <template>
                <table class="table table-striped table-bordered table-advance curHoldingTable pc_table"
                  ...
                  <tr class="tb_tb_tr" v-for="item1 of financial">
                    ...
                    <td class="mb-td" v-if="item1.k0!=undefined">
                      <div class="data_box2">{{item1.k0 | setNum}}</div>
                    </td>
                    <td class="mb-td" v-if="item1.k1!=undefined">
                      <div class="data_box2">{{item1.k1 | setNum}}</div>
                    </td>
                    <td class="mb-td" v-if="item1.k2!=undefined">
                      <div class="data_box2">{{item1.k2 | setNum}}</div>
                    </td>
                    <td class="mb-td" v-if="item1.k3!=undefined">
                      <div class="data_box2">{{item1.k3 | setNum}}</div>
                    </td>
                    <td class="mb-td" v-if="item1.k4!=undefined">
                      <div class="data_box2">{{item1.k4 | setNum}}</div>
                    </td>
                  </tr>
                  </tbody>
                </table>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /*查找自选股*/
      searchSelfStock() {
        this.$http.get(httpUrl.diagnosticStocksSearchApi, {}).then(function (res) {
          console.log(res)
          if (res.body.code == 0) {
